...
def fetch_nps_comments(service, start_date, end_date, top_n=30):
    """
    Busca os principais comentarios do evento nps_response nos ultimos N dias.
    Retorna lista de dicts {texto, categoria, count} ordenada por count desc.

    Requer dimensao customizada 'nps_comment' e 'nps_number' configuradas
    no evento 'nps_response' na propriedade GA4 do App.
    """
    body = {
    ...
    try:
        ...
        print(f"  AVISO: nao foi possivel buscar comentarios NPS (app) - {e}")
        results = []
    return results
